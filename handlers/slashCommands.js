...
module.exports = (client) => {
  ...
  try {
    console.log(`${String("[x] :: ".cyan)}Now loading the Slash Commands ...`.brightBlue)
    let allCommands = [];
      ...
      if (config.loadSlashsGlobal) {
        client.application.commands
          ...
          .then((slashCommandsData) => {
            console.log(`[x] :: `.cyan + `LOADED ${slashCommandsData.size} SLASH COMMANDS (with ${slashCommandsData.map((d) => d.options).flat().length} subcmds in total) after: `.brightBlue + `${Date.now() - dateNow}ms`.green)
            console.log(
          ...
          .forEach(async (guild) => {
            try {
              ...
              guild.commands
                .set(allCommands)
                .then((slashCommandsData) => {
                  console.log(`[x] :: `.cyan + `LOADED ${slashCommandsData.size} SLASH COMMANDS (with ${slashCommandsData.map((d) => d.options).flat().length} subcmds in total) Loaded for: ${`${guild.name}`.underline} after: `.brightBlue + `${Date.now() - dateNow}ms`.green)
                })
                .catch(console.error);
            } catch (e) {
